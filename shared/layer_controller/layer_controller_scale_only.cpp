...
array<layer_properties> initialize_layers(array<layer_properties> layers) {
	...
	for(int i=layers.length(); i<NUM_LAYERS; i++) {
		layers.insertLast(layer_properties(DEFAULT_SCALE[i], true));
...
class script {
	...
	void apply_layer_changes(bool reorder_layers, bool scale_layers, bool hide_layers) {
		...
		for(int i=0; i<NUM_LAYERS; i++) {
			...
			if(reorder_layers) {
				int pos=i;
				while(pos>0 and active_layers[i].scale < active_layers[visual_layer[pos-1]].scale) {
...
class layer_properties {
	...
	layer_properties() {
		layer_properties(1, true);
	}
	
	layer_properties(float size, bool vis) {
		scale = size;
		visible = vis;
	}
	
	layer_properties@ clone() {
		return layer_properties(scale, visible);
	}
}